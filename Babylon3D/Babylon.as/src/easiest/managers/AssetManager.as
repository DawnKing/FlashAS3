package easiest.managers
{
    ...
    import easiest.unit.asserts.assertNotNull;
    import easiest.unit.asserts.assertNull;
...
//            Log.debug1("空闲下载:url="+asset.url, CLASS_NAME);
        ...
        public static function processTest():void
        {
            ...
            assertTrue(m_assetLoaderPool.size >= 0);
            
            // 测试正在加载的AssetLoader
            for (url in m_waitLoadList) 
            {
                assetLoader = m_waitLoadList[url];
                assertEquals(url, assetLoader.url);
                assertFalse(url in m_loadingList);
            }
            ...
            assertTrue(loadCount <= MAX_LIMIT);
            ...
            while (m_assetLoaderPool.size > 0)
            {
                ...
                assertEquals(assetLoader.callbackList.length, 0);
                assertEquals(assetLoader.assetType, "");
                assertEquals(assetLoader.cacheType, 0);
                assertFalse(assetLoader.isLoading());
     
                assertNull(assetLoader.onComplete);
                assertNull(assetLoader.urlLoader);
                assertNull(assetLoader.loader);
                assertNull(assetLoader.assetData.asset);
                assertFalse(assetLoader.hasCache());
                assertFalse(assetLoader.isCache);
...
//            case AssetType.SPRITE_SHEET:// 序列帧不需要缓存，由SpriteAnimation缓存
    ...
    public static function processTest():void
    {
        assertTrue(m_urlLoaderPool.size >= 0);
        assertTrue(m_urlLoaderPool.size <= MAX_LIMIT);
        assertTrue(m_loaderPool.length <= MAX_LIMIT);
        assertTrue(m_loaderBytesPool.length <= MAX_LIMIT);        
    }
