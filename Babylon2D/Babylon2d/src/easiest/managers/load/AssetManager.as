package easiest.managers.load
{
	...
	public final class AssetManager
	{
        ...
        public static const MAX_LIMIT:int = 1;	 // 限制同时最多下载，其余的进入等待下载队列
		...
		public static function load(url:String, callback:Function, assetType:String, cacheType:int = AssetType.CACHE_SOMETIMES, param:Object = null):void
		{
            ...
            if (addLoading(url, callback, param))
                return;
        ...
        private static function addLoading(url:String, callback:Function, param:Object):Boolean
        {
            // 如果是错误的资源，不重复加载
            if (AssetLoader.errorList.indexOf(url) != -1)
            {
        ...
        private static function cacheHandler(assetLoader:AssetLoader):void
        {
            ...
            switch (assetLoader.cacheType)
            {
                ...
                case AssetType.CACHE_SOMETIMES:
                    ...
                    assetLoader.timer = FrameManager.time;
                    break;
        ...
        {
            var time:int = FrameManager.time - DISPOSE_SOMETIMES_CACHE;
...
import easiest.core.Log;
import easiest.managers.load.LoaderBytes;
import easiest.managers.load.LoaderExtend;
import easiest.managers.load.AssetData;
import easiest.managers.load.AssetType;
import easiest.managers.ObjectPool;
import easiest.managers.load.URLLoaderExtend;
import easiest.debug.Assert;
...
final class AssetLoader implements IClear
{
