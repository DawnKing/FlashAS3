package game.world.map.view
{
	 ...
	 *
	 */
	public final class MapCell extends Sprite2D
	{
		/**允许的最大耗时*/
		...
		/**地图切片任务集中列表*/
		private static const _mapPartTasks:Vector.<MapCell>=new Vector.<MapCell>();
		...
		public static function get loadingCount():uint
		{
			...
			{
				if (_mapPartTasks[i] && _mapPartTasks[i].status < STATUS_TEXTURE_READYED)
					count++;
			}
			return count;
		}
		 ...
		 *
		 */
		private static function addTask(mp:MapCell):void
		{
			//从任务列表移除
			mp.isNoAction=false;
		 ...
		 *
		 */
		private static function removeTask(mp:MapCell):void
		{
			//从任务列表移除
			var idx:int=_mapPartTasks.indexOf(mp);
			if (mp)
		...
		public static function updateTasks(realCurrTime:uint, avgMs:int):void
		{
			for (var i:int=0; i < _mapPartTasks.length; i++)
			{
				var ms:int=(FrameManager.time - realCurrTime);
				if (ms > (avgMs + MAX_FRAME_MS)) //耗时已经超过了3毫秒,就退出
				{
					return;
				}
				var mp:MapCell=_mapPartTasks[i];
				if (!mp)
				{
...
//		public static const STATUS_UNCOMPRESS:uint=8;
		 ...
		 * 地图切片
		 */
		public function MapCell(url:String, pkey:uint, partW:int, partH:int)
		{
			super();
...
//                status=STATUS_UNCOMPRESS;
		...
		public function removeTextureBuffer():void
		{
			...
			if (status > STATUS_TEXTURE_READYED)
			{
				status=STATUS_TEXTURE_READYED;
				removeTask(this);
			}
