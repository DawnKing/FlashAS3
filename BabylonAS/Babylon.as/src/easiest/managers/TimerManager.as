package easiest.managers
{
	...
	public final class TimerManager
	{
		 ...
		 * @param interlval 间隔时间（毫秒）
         * @param repeatCount (default = 0) — 指定重复次数。如果为 0，则计时器重复无限次数。如果不为 0，则将运行计时器，运行次数为指定的次数，然后停止。 
		 * @param keepRate 帧补偿
		 */
		public static function add(updateFunction:Function, interlval:int, repeatCount:int = 0, keepRate:Boolean = false):void
		{
            ...
            timeData.intervalTime = interlval;
            timeData.nextTime = FrameManager.timer + interlval;
		...
		{
            var currTime:int = FrameManager.timer;
